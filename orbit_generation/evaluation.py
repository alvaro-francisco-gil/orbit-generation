...
# %% auto 0
__all__ = ['DISTANCE_FUNCTIONS', 'plot_metric_heatmaps', 'find_non_matching_elements', 'evaluate_clustering_multiple_labels',
           'euclidean_distance', 'manhattan_distance', 'cosine_distance', 'dtw_distance', 'calculate_distance',
...
def plot_metric_heatmaps(results: dict,  # Dictionary containing results for each combination of distance metric and clustering algorithm
    ...
    """
    Plot heatmaps for each evaluation metric showing scores across different distance metrics
    and clustering algorithms combinations.
    """
...
def find_non_matching_elements(main_array: np.ndarray,  # The main array with larger set of elements
                             check_array: np.ndarray,  # The array with elements to check against the main array
                             ) -> np.ndarray:
    """
...
def evaluate_distance_metrics_and_clustering(
    orbit_data: np.ndarray,  # Orbit data as multivariate time series [n_samples, n_features, n_time_steps] or point data [n_samples, n_features]
    true_labels: np.ndarray,  # Array of true labels for the orbit data
    distance_metrics: Optional[list] = None,  # List of distance metrics to use. If None, uses all available metrics
    clustering_algorithms: Optional[list] = None,  # List of clustering algorithms to use. If None, uses all available algorithms
    evaluation_metrics: Optional[list] = None,  # List of evaluation metrics to use. If None, uses all available metrics
    n_clusters: Optional[int] = None,  # Number of clusters. If None, inferred from labels
...
def machine_learning_evaluation(X: np.ndarray,  # Features array, expected to be 2D. Will attempt to reshape if higher dimensions
                              ...
                              ) -> Union[Dict, Tuple[Dict, Pipeline]]:
    """
