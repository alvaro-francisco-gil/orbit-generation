...
![orbit-generation-pic](./index_files/orbit-generation.jpg)
...
``` python
visualize_static_orbits(data= orbit_data,
                        orbit_indices=[315,120,70,180,190],
                        point_dict=EM_POINTS)
```
...
visualize_static_orbits(data= orbit_data,
                        time_instants=[0,50],
                        orbit_indices=[40],
                        plot_reference_box=False)
```

![](index_files/figure-commonmark/cell-12-output-1.png)

``` python
export_dynamic_orbits_html(data=orbit_data,
                           filename='../data/example_training_data/example_orbits_visualization.html')
```
...
``` python
plot_histograms_comparison(data1=orbit_data[0:200,1:],
                           data2=orbit_data[200:400,1:],
                           normalize=True)
```
...
``` python
errors = calculate_errors(orbit_data=orbit_data,
                           mu=EM_MU,
                           orbit_indices = [0])
```
...
``` python
calculate_pairwise_orbit_distances(orbit_data1=orbit_data[0:1],
                                   orbit_data2=orbit_data[1:2],
